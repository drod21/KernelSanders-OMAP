#!/bin/sh
T=$PWD
PRODUCT=KangBang-Tuna
PRODUCT_CONFIG_FILE=kangbang_defconfig
TOOLCHAIN=tools/toolchain/arm-linux-androideabi-4.4.x/bin/arm-linux-androideabi-
...
make ARCH=arm CROSS_COMPILE=$CCOMPILER -j50
...
cd $T/out/$PRODUCT
zip -r $PRODUCT-$VERSION-$DATE.zip META-INF/ system/ kernel/ flash_image
